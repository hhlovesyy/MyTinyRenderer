...
​	在很多渲染器的实现中（如比较出名的TinyRenderer），最终渲染得到的结果是以tga格式文件保存的，这其实不利于我们实时查看自己渲染器写的效果是否正确，因此在一开始我们就先搭建一个简易的窗体程序，便于后续将渲染得到的结果呈现在屏幕上，以及后面会根据相机进行缩放、平移等。这里面主要使用C++的windows相关API接口来创建窗口，这部分不需要完全掌握，直接把代码和核心的注释贴在下面：
...
这里color_buffer是char类型的数组，原因是颜色范围在0到255之间，因此用char类型保存是完全够用的（注意这里我们还没有使用HDR，暂且先LDR即可）。
...
![image-20240803163753720](./assets/image-20240803163753720.png)

不难看出，实际上重心坐标就等于上图左下角那两个vector3的叉乘结果（当然要归一化z到1）。只有当三角形的三个重心坐标都>0时才认为是在三角形内（这里一概不过多考虑边界情况，比如=0之类的）。趁热打铁直接来看核心代码，首先是求解三角形重心坐标的代码：
...
这里的factor，s，t其实就是一步叉乘的结果，我们来推导一下（读者可以自行验算一下，公式应当是正确的）：

<img src="./assets/B84311D147F1F833D00081F811265E8F.png" alt="B84311D147F1F833D00081F811265E8F" style="zoom:67%;" />
...
![image-20240803192847280](./assets/image-20240803192847280.png)

也就是说，一个能想到的加速方法是只遍历上图当中绿色矩形的区域，也就是三角形的包围盒（AABB），这里的AABB求解代码如下：
...
> 注：在整个渲染器的实现中，我们并不会在文档中给出所有函数的实现（因为量比较大），例如上面的`vec2_min`函数的实现，不过这种函数字面意思一般都能理解，大部分这类函数都在`maths.h`文件和`maths.cpp`文件中可以找到定义。
